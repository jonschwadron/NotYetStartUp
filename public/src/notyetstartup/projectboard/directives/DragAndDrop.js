...
.directive('dragContainer', function() {
  return {
    ...
    link: function($scope, $element, $attrs, dragContainer) {
      ...
      $element.on('dragstart', dragContainer.handleDragStart.bind(
        dragContainer));
      $element.on('dragend', dragContainer.handleDragEnd.bind(
        dragContainer));
      // $element.on('dragenter', dragContainer.handleDragEnter.bind(dragContainer));

      $scope.$watch($attrs.dragContainer, dragContainer.updateDragData.bind(
        dragContainer));
      $attrs.$observe('mimeType', dragContainer.updateDragType.bind(
        dragContainer));
...
.directive('dropContainer', function($document, $parse) {
  return {
    ...
    link: function($scope, $element, $attrs, dropContainer) {
      ...
      $scope.$watch($attrs.accepts, dropContainer.updateMimeTypes.bind(
        dropContainer));
...
.controller('DropContainerController', function($dragging) {
  ...
  dropContainer.addDropTarget = function(anchor, dropTarget) {
    if (validAnchors.indexOf(anchor) < 0) throw new Error(
      'Invalid anchor point ' + anchor);
    if (targets[anchor]) throw new Error(
      'Duplicate drop targets for the anchor ' + anchor);
  ...
  dropContainer.updateDragTarget = function(e, skipUpdateTarget) {
    ...
    if (!skipUpdateTarget) {
      angular.forEach(targets, function(dropTarget, anchor) {
        ...
        var distanceSq = Math.pow(anchorX - e.offsetX, 2) + Math.pow(
          anchorY - e.offsetY, 2);
  ...
  dropContainer.handleDragEnter = function(e) {
    ...
    if (!dropContainer.accepts || dropContainer.accepts.indexOf($dragging
        .getType()) >= 0) {
      e.preventDefault();
    } else {
      return;
...
.directive('dropTarget', function($parse) {
  return {
    ...
    link: function($scope, $element, $attrs, ctrls) {
      ...
      var destroy = dropContainer.removeDropTarget.bind(dropContainer,
        anchor);
